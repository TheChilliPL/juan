...
export class JsonVault<T extends (EmptyObject extends T ? {} : never)> extends Vault<T> {
    ...
    protected async loadImpl(): Promise<void> {
        try {
            console.debug("Loading vault", this.path);
            let data = await fs.readFile(this.path, "utf8");
            this._data = JSON.parse(data);
            console.log("Loaded vault", this.path);
        } catch (e: any) {
            if (e?.code == "ENOENT") {
                this._data = {} as T;
                console.debug("Created new vault", this.path);
            } else {
                throw e;
            }
        }
